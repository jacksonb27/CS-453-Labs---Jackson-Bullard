...
1. What is the difference between a TCP message and an HTTP request?
    # A TCP message is the "broken up" or "segmented" message(s) from a stream of data, usually from the HTTP request being the data stream.

2. What does the `Content-Type: application/json` header tell the server?
    # That header gives the server information on what the type of data is going to be. The data is then sent back to the client.

3. Why should a server return different HTTP status codes for different situations?
    # Problems are easier to diagnose when you have different codes for different things. If every issue had the same code, fixing it would be tough.

4. What happens if the client sends invalid JSON?
    # Typically, the server would send back an error code using HTTP.

5. How is this lab different from Lab 1?
    # This lab is focused more on HTTP, TCP, and JSON information, rather than client/server communication. We are working with functions and 
