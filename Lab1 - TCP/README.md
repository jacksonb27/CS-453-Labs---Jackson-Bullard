...
1. What is the difference between the client and the server?
    # The client is the local machine/instance that is running the program, and the server is the "main" hub of the program. A client
    # communicates to the server to exchange information about a session.

2. Why does the server need to keep running after handling one request?
    # The server needs to keep running in order to accept more requests as well as send back the needed information to the client.

3. What happens if two clients connect at the same time?
    # Each client is given a unique instance identifier to differentiate the two, and the server handles the requests of both.

4. How is this different from HTTP?
    # HTTP is a "language" that data can be shared in. Clients/servers are "minds" or "hubs" that can process this information.


What each protocol does:
    # ECHO = returns the message sent
    # UPPER = returns the message in all upper case
    # LOWER = returns the message in all lower case
    # REVERSE = returns the message in reverse order
    # TIME = returns server's uptime
